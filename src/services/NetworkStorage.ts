...
export class NetworkStorage {
  ...
  /**
   * Store a single network request
   */
  async storeRequest(request: NetworkRequest): Promise<void> {
    return this.storeRequests([request]);
  }
  ...
  /**
   * Store a single network error
   */
  async storeError(error: NetworkError): Promise<void> {
    return this.storeErrors([error]);
  }
  ...
  async cleanupStorage(): Promise<void> {
    // Default cleanup with 7 days retention
    await this.cleanup(7);
  }

  /**
   * Get recent network requests (most recent first)
   */
  async getRecentRequests(limit: number = 50): Promise<NetworkRequest[]> {
    if (!this.db) {
      throw new Error("Storage not initialized");
    }
  ...
  async getStatistics(): Promise<{
    ...
    return {
      ...
      storageSize: stats.estimatedSize,
    };
  }
